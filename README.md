...
各モデル用のプロンプトは `autoimagegenerator/prompts/<モデル名>/` 以下の JSON ファイルで管理されています。

### 必須ファイル

1. **positive_base.json**
   - 基本的な positive プロンプトを定義
   - 必ず含める必要のあるプロンプト（画質設定、人物の基本設定など）を記述
   - 例: 画質設定、人物の体型、髪型、髪の色など
...
3. **positive_pose.json**
   - ポーズに関する positive プロンプトを定義
   - 人物のポーズや表情に関する設定を記述

4. **positive_optional.json**
   - オプションの positive プロンプトを定義
   - 服装、アクセサリー、カメラ設定、場所、ライティングなどを記述

5. **positive_selfie.json**
   - セルフィー撮影時の positive プロンプトを定義
   - セルフィーモード時のみ使用

6. **cancel_seeds.json**
   - 生成をキャンセルする Seed 値のリストを定義
   - 特定の Seed 値で生成される画像の質が低い場合に使用

7. **positive_cancel_pair.json**
   - 同時に使用できないプロンプトの組み合わせを定義
   - 例: 「ビキニ」と「ベッドルーム」など、不適切な組み合わせを防ぐ

### JSON ファイルの構造

各 JSON ファイルは以下の構造を持ちます：
...
```json
{
  "カテゴリ名": {
    ...
    "use max prompts": 数値, // このカテゴリから最大いくつのプロンプトを使用するか
    "use min prompts": 数値  // このカテゴリから最小いくつのプロンプトを使用するか
  }
}
```

- `use max prompts` と `use min prompts` の間でランダムな数のプロンプトが選択されます
- 値が 0 の場合、そのカテゴリからプロンプトは選択されません
- 1 以上の場合、指定された範囲内でランダムにプロンプトが選択されます

### プロンプトの優先度

プロンプトの重みづけは `:1.2` のような形式で指定可能です：
```json
"prompts": [
